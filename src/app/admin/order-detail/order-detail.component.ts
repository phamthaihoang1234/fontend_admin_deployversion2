...
export class OrderDetailComponent implements OnInit {
  ...
  deliver() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let chatMessage : any;
        chatMessage = new ChatMessage('-1', ' Đơn hàng của bạn đã được xác nhận rồi đấy !');
  ...
  cancel() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let chatMessage : any;
        chatMessage = new ChatMessage('-1', ' Đơn hàng của bạn đã được hủy thành công !');
  ...
  confirm() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let chatMessage : any;
        chatMessage = new ChatMessage('-1', ' Đơn hàng của bạn đã được thanh toán thành công  !');
