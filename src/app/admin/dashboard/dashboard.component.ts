...
export class DashboardComponent implements OnInit {
  ...
  getRevenueMonthNow(): number {
    let revenue = 0
    console.log("So luong order getRevenueMonthNow thu duoc la: "+ this.orders.length);
    for (let i = 0; i < this.orders.length; i++) {
    ...
    console.log("doanh thu thang la: "+ revenue);
    return revenue;
  }
  ...
  getAllOrder() {
    this.orderService.get().subscribe(data => {
      ...
      console.log("So luong order thu duoc la: "+ this.orders.length);
      this.orderHandle = 0;
      for (let i = 0; i < this.orders.length; i++) {
